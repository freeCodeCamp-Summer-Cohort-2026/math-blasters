...
function requireStringArray(
  ...
  for (let i = 0; i < arr.length; i++) {
    if (typeof arr[i] !== 'string') {
      throw new Error(
        `${path}, field "${field}": Expected every element to be a string, but element at index ${i} is a ${typeof arr[i]}.`,
      );
    }
  }
}
...
function parseStep(
  ...
  if (section.marker === 'explain') {
    return { type: 'explain', content: section.content };
  }
...
export function parseLesson(source: string, path: string): Lesson {
  ...
  const fm = parseFrontmatter(rawYaml, path, expectedSlug);
