...
// ---------------------------------------------------------------------------
// Mock parseCriteria so we can assert it's called without needing a real
// implementation. validateLesson on main is already a real implementation,
// so we don't mock it — we let it run.
// ---------------------------------------------------------------------------
...
describe('parseLesson — happy paths', () => {
  it('parses a tutorial lesson with the correct step structure', () => {
    ...
    if (lesson.steps[0].type === 'explain') {
      expect(lesson.steps[0].content).toContain('Adding means putting two amounts together.');
    }

    expect(lesson.steps[1].type).toBe('answer');
    if (lesson.steps[1].type === 'answer') {
      expect(lesson.steps[1].prompt).toContain('What is $3 + 4$?');
  ...
  it('parses a lab lesson with outcome and requires', () => {
    ...
    expect(lesson.slug).toBe('practice-addition');
    expect(lesson.title).toBe('Practice addition');
    expect(lesson.type).toBe('lab');
    expect(lesson.outcome).toBe('Can add single-digit numbers fluently');
